...
import type { Chamber, Facility } from '@/types/models';
...
export default function ChambersPage() {
  ...
  // Detect first facility
  const { data: facilities } = useApiQuery<Facility[]>('/facilities');
  const facilityId = facilities?.[0]?.id || '';

  // Load chambers for detected facility
  const { data: chamberData, loading, refetch } = useApiQuery<Chamber[]>(
    facilityId ? '/chambers' : null,
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setShowAddModal(false); setForm(emptyChamberForm); refetch();
